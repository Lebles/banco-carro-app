...
export default function VerifyPage({session, setSessionVar}) {
  return (<ScrollView>
    <View style={{alignItems: 'center', marginTop: status_bar_height}}>
      <Text style={{fontSize: 18, width: '90%', borderRadius: 10, backgroundColor: '#006647', color: '#ffd100', padding: 10, textAlign: 'justify'}}><Text style={{fontWeight: 'bold'}}>Ultimo Paso:</Text> Confirme que toda la informacion de la solicitud es correcta.</Text>
      ...
      <View style={{width: '90%'}}>
        ...
        <InfoRow title='Telefono Residencial' info={session.user.tel1} />
        <InfoRow title='Telefono de Oficina' info={session.user.tel2} />
        <InfoRow title='Correo Electronico' info={session.user.correo} />
      ...
      <View style={{width: '90%'}}>
        ...
        <InfoRow title='Condicion' info={session.carro.condicion} />
      </View>

      <Text style={styles.h1}>Prestamo</Text>
      <View style={{width: '90%'}}>
        <InfoRow title='Monto solicitado' info={session.halfForm.solicita} />
        <InfoRow title='Plazo de pago' info={session.halfForm.plazo} />
        <InfoRow title='Periodo de taza fija' info={session.halfForm.periodo} />
      </View>
